...
export default function ConfusionMatrix({ data, x_labels, y_labels }) {
  console.log(data, x_labels, y_labels);
  /*--------------------------------------------------------------------------------
  ...
  function fetchDataWindowResults(d, testLabel, predLabel, epoch, conf) {
    console.log("DDDDD", d, Object.entries(d));
    ...
    let selectedEntries = [];
    for (let i = 0; i < Object.entries(d).length; i++) {
      let dataPoint = Object.entries(d)[i];
  ...
  /*--------------------------------------------------------------------------------
      ...
      --------------------------------------------------------------------------------*/
  function addAxisLabels() {
    console.log("ADD AXIS LABELS=====", x_labels, y_labels);
    let xaxis_labels = x_labels;
    let yaxis_labels = y_labels;
  ...
  /*--------------------------------------------------------------------------------
      ...
      Returns: N/A
      --------------------------------------------------------------------------------*/
  function clickedRect(d_on, d, currentEpochSetting, currentConfSetting) {
    let actual = d_on[0];
  ...
  /*--------------------------------------------------------------------------------
        ...
        Returns: N/A
        --------------------------------------------------------------------------------*/
  function emptyMatrix(counters, tableDimension, ycounters, svg) {
  ...
  /*--------------------------------------------------------------------------------
        ...
        Returns: N/A
        --------------------------------------------------------------------------------*/
  function refineChoice(dataset, currentEpochSetting, currentConfSetting) {
    let datasubset = [];
  ...
  useEffect(() => {
    ...
    /*--------------------------------------------------------------------------------
            INITIALIZING MATRIX
     --------------------------------------------------------------------------------*/
    // Filter based on default confidence score
    ...
    d3.selectAll(".slider").on("change", function () {
      ...
      emptyMatrix(counters, tableDimension, ycounters, svg); // Remove all current rects from matrix
      refineChoice(dataset, currentEpochSetting, currentConfSetting); // Filter data for changes in min confidence score
      rect = svg
        .selectAll("rect") // Defining rect as child of SVG
        .data(datasubset)
        .enter()
